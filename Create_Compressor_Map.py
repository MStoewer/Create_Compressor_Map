...
def run_map(stepList, timesteps, trace_control_cmd, nodes, cpus, mem, gmcPlay_exe, POST_EXE, start_point):
    ...
    for sim_number, pressure in enumerate(stepList):
        ...
        os.system('mkdir %02i_4AV_n17100_p60kPa_p%ikPa_Ttot288'%(i, folderName[i]))
        os.chdir('./%02i_4AV_n17100_p60kPa_p%ikPa_Ttot288'%(i, folderName[i]))
        cur_wd = os.getcwd()
        ...
        os.chdir('./run')
        os.system("sed -i s/REPLACE_JOB/%02i_4AV_n17100_p60kPa_p%ikPa_SST_GammaReTheta_Referenz/g job_TRACE.sh"%(i, folderName[i]))
        ...
        if i != 0:
            ...
            os.system('cp ../../%02i_4AV_n17100_p60kPa_p%ikPa_Ttot288/output/cgns/TRACE.cgns ./'%(i-1, folderName[i-1]))
            os.system('cp ../../%02i_4AV_n17100_p60kPa_p%ikPa_Ttot288/input/TRACE_entry.input ./'%(i-1, folderName[i-1]))
            ...
            f = open('%s/%02i_4AV_n17100_p60kPa_p%ikPa_Ttot288/output/residual/residual.dat'%(script_path, i-1, folderName[i-1]), 'r')
        ...
        os.chdir(script_path + '/%02i_4AV_n17100_p60kPa_p%ikPa_Ttot288'%(i, folderName[i]))
        print(os.getcwd())
...
def continue_map(sim_number, timesteps, trace_control_cmd, nodes, cpus, mem, gmcPlay_exe, folderName):
        ...
        for n in range(i, i+5):
            ...
            while os.path.isfile('./run/log.run') is False:
                time.sleep(600)
                continue
            if open('./run/log.run','r').read().find('with errorcode 1') is True:
                print('Simulation not converged - Switching to mass flow BC')
                return [1, i]
...
def create_map_file(script_path,POST_EXE):
    ...
    with open('map_data.dat','w+') as f:
        f.write('Back Pressure[Pa] MassFlow[kg/s] MassFlow_corr[kg/s] PI_tot EffPoly[%] EffIsen[%] Ma_in T_in[K]\n')
    ls
    ...
    for sim_dir in subfolders:
        if os.path.exists(sim_dir + '/post'): 
            ...
            PI_tot  = re48gbad_d0_variable(postdir,'PressureStagnationAbsRatio',d0type='relations')
            ...
            with open('map_data.dat','a+') as f:
                f.write(str(Back_pressure) + ' ' + str(MassFlow) + ' ' + str(MassFlow_corr) + ' ' + str(PI_tot) + ' '  + str(EffPoly) + ' ' + str(EffIsen) + ' ' +  str(Ma_in) + ' ' + str(T_in) + '\n')
            


script_path = os.path.dirname(os.path.realpath(__file__))
bigwork = ('/bigwork/nhkcmast/10_MA/07_4AV_steady_SST_GammaReTheta_KatoLaudner_Referenz/')
